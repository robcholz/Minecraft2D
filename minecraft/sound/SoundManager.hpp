...
class SoundManager {
 public:
  ...
  // using std::optional<> here since some of the blocks might not have yet
  // bound to a sound object
 ...
 private:
  ...
  void updateSoundSettings() {
    if (updateSignalQueue.empty())
      return;
    musicSoundEventPlayer.setVolume((float)soundLevel.masterVolume *
                                    (float)soundLevel.music / 100);
    soundEventPlayer.setVolume(soundLevel.masterVolume);
    updateSignalQueue.pop();
  }

  void updateMusicSoundEvent() {
    if (musicSoundEventCurrent != nullptr &&
        musicSoundEventPlayer.getStatus() == sf::Sound::Status::Stopped) {
      musicSoundEventQueue.pop();
    ...
    if (musicSoundEventPlayer.getStatus() == sf::Sound::Status::Stopped) {
      ...
      musicSoundEventPlayer.setBuffer(
          musicSoundEventCurrent->getSoundEvent().getSound());
      musicSoundEventPlayer.play();
      PLOG_DEBUG << "Current playing: "
                 << musicSoundEventCurrent->getSoundEvent().getID().toString();
    }
  }

  void updateSoundEvent() {
    if (soundEventCurrent != nullptr &&
        soundEventPlayer.getStatus() == sf::Sound::Status::Stopped) {
      soundEventQueue.pop();
