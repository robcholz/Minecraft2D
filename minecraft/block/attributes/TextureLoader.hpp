...
class TextureLoader {
 ...
 public:
  ...
  sf::Texture* getBlockTextureTile(Identifier& identifier,
                                   coordinate::DirectionType type) {
 ...
 private:
  std::map<std::pair<String, coordinate::DirectionType>, sf::Texture*>
      textureTiles;
  Json blockStateJson;
  std::map<coordinate::DirectionType, String> directionMap = {
      {coordinate::DirectionType::UP, "up"},
  ...
  sf::Texture* loadBlockTiles(Identifier& identifier,
                              coordinate::DirectionType direction) {
    std::ifstream file(identifier.getAbsolutePath());
    blockStateJson = Json::parse(file);
    auto texture = new sf::Texture;
    texture->loadFromFile(FileHelper::transferJsonPathToFilePath(
        blockStateJson[directionMap[direction]]));
