...
public:
	...
	void updateLightSource(const coordinate::BlockPos& blockPosition, uint8_t lightLevel) override {
	...
	void updateDaylight(ChunkPosT chunkPos) {
		for (auto x_in_chunk = 0; x_in_chunk < chunk::ChunkGenSettings::CHUNK_WIDTH; ++x_in_chunk) {
			...
			for (auto z_pos = worldChunkAccess->getChunk(chunkPos)->getHeightMap(x_in_chunk); z_pos < chunk::ChunkGenSettings::CHUNK_HEIGHT; ++z_pos) {
	...
	void update() {
		while (!lightUpdatesQueue.empty()) {
			...
			propagateLight(lightUpdate.first.x, lightUpdate.first.z, lightUpdate.second);
...
private:
	...
	void propagateLight(BlockPosT x, BlockPosT z, uint8_t lightLevel) {
		...
		auto render_interval = worldChunkAccess->getChunkStream()->getRenderInterval();
		// check if the block is in the render interval
		if (!(render_interval.lower < chunk_pos.chunkPos || render_interval.upper > chunk_pos.chunkPos))
			return;
		// check if the block position is in the range
		if (chunk_pos.blockPos.x < 0 || chunk_pos.blockPos.x >= chunk::ChunkGenSettings::CHUNK_WIDTH
		    || chunk_pos.blockPos.z < 0 || chunk_pos.blockPos.z >= chunk::ChunkGenSettings::CHUNK_HEIGHT)
			return;
