...
struct ChunkDataPacket {
  // note: since we know the position of the chunk and all the blocks in vector
  // are stored linearly we don't need to specify the position of each block
  coordinate::ChunkPositionT chunkPos;  // the position of the chunk
  std::vector<block::ID::SerialIDT>
      serialIDContainer;  // to identify the type of the block
  std::vector<Direction::DirectionT>
      blockDirectionContainer;  // to identify the direction of the block
  std::vector<TileColor::TileColorT>
      tileColorContainer;  // to store the light level
};

template <typename vT>
// serialization func implementation
void serialize(vT& s, ChunkDataPacket& o) {
  constexpr auto container_size = chunk::ChunkGenSettings::CHUNK_WIDTH *
...
class ChunkDataPacketAdapter {
 private:
  ...
  static ChunkPosT toOneDimensionIndex(ChunkPosT x, ChunkPosT y) {
    return y * ChunkGenSettings::CHUNK_WIDTH + x;
  }

 private:
  friend bitsery::Access;

  static void allocateContainerMemory(ChunkDataPacket* chunkDataPacket,
                                      ChunkPosT capacity) {
  ...
  template <typename vT>
  static void addToContainer(std::vector<vT>* vector,
                             ChunkPosT x,
                             ChunkPosT y,
                             vT value) {
 ...
 public:
  ...
  static void compress(const Chunk& chunk, ChunkDataPacket* chunkDataPacket) {
    ...
    allocateContainerMemory(
        chunkDataPacket,
        ChunkGenSettings::CHUNK_WIDTH * ChunkGenSettings::CHUNK_HEIGHT);
    for (ChunkPosT x_pos = 0; x_pos < ChunkGenSettings::CHUNK_WIDTH; x_pos++) {
      for (ChunkPosT y_pos = 0; y_pos < ChunkGenSettings::CHUNK_HEIGHT;
        ...
        addToContainer<DirectionT>(
            &chunkDataPacket->blockDirectionContainer, x_pos, y_pos,
  ...
  static std::unique_ptr<Chunk> decompress(ChunkDataPacket* chunkDataPacket) {
    ...
    Direction::DirectionT directions[ChunkGenSettings::CHUNK_WIDTH]
                                    [ChunkGenSettings::CHUNK_HEIGHT];
    TileColor::TileColorT tile_colors[ChunkGenSettings::CHUNK_WIDTH]
                                     [ChunkGenSettings::CHUNK_HEIGHT];
    for (ChunkPosT x_pos = 0; x_pos < ChunkGenSettings::CHUNK_WIDTH; ++x_pos) {
      for (ChunkPosT y_pos = 0; y_pos < ChunkGenSettings::CHUNK_HEIGHT;
           ++y_pos) {
        auto oneDimensionIndex = toOneDimensionIndex(x_pos, y_pos);
        auto serialID = static_cast<block::ID::SerialIDT>(
            chunkDataPacket->serialIDContainer[oneDimensionIndex]);
        auto block = block::Blocks::getInstance().createObject(serialID);
        blocks[x_pos][y_pos] = block;
        directions[x_pos][y_pos] =
            chunkDataPacket->blockDirectionContainer[oneDimensionIndex];
        tile_colors[x_pos][y_pos] =
