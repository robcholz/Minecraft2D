...
namespace coordinate {
	...
	public:
		...
		static DesPosT convertTo(SrcPosT position) {
			...
			if constexpr (std::is_same<EntityPos, SrcPosT>::value && std::is_same<PixelPos, DesPosT>::value) {
				auto zoom = RenderSystem::Settings::pixelProportion;
				return PixelPos(math::floor(position.x * zoom), -math::floor(position.z * zoom));
			}
			if constexpr (std::is_same<PixelPos, SrcPosT>::value && std::is_same<BlockPos, DesPosT>::value) {
				auto zoom = (float) RenderSystem::Settings::pixelProportion;
				return BlockPos(math::floor(position.x / zoom), -math::floor(position.z / zoom));
			}
			if constexpr (std::is_same<PixelPos, SrcPosT>::value && std::is_same<EntityPos, DesPosT>::value) {
				auto zoom = (float) RenderSystem::Settings::pixelProportion;
			...
			return DesPosT(static_cast<typename DesPosT::type>(position.x), static_cast<typename DesPosT::type>(position.z));
		}
