...
class MinecraftVersion {
 ...
 public:
  ...
  static String getReleaseTarget() { return releaseTarget; }

  static String getProductionName() {
    return getName() + " " + getReleaseTarget();
  }

  static int getWorldVersion() { return worldVersion; }

  static int getProtocolVersion() { return protocolVersion; }

  static int getPackVersion() { return packVersion; }

  static String getBuildTime() { return buildTime; }

  template <typename T>
  static T isStable() {
    static_assert(
        std::is_same<T, bool>::value || std::is_same<T, String>::value,
