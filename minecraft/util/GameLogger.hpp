...
class GameLogger {
 ...
 protected:
  ...
  static String getCurrentDate() {
    ...
    return std::to_string(1900 + t->tm_year) + "-" +
           std::to_string(1 + t->tm_mon) + "-" + std::to_string(t->tm_mday);
  }
 ...
 public:
  ...
  static void init() {
    ...
    if (isLoggedToFile()) {
      ...
      plog::init(logSeverity(), &console_appender);
  }
  ...
  static String getLogFileName() {
    ...
    int current_days = (date_ptr->tm_year + 1900) * 360 +
                       (date_ptr->tm_mon + 1) * 30 + date_ptr->tm_mday;
    ...
    return logPath + "/" + getCurrentDate() + "-" +
           std::to_string(daily_max + 1) + "-log" + ".log";
  }
 ...
 private:
  ...
  static bool isLoggedToFile() { return optionsJson["logging"]["file"]; }
   ...
   * @return split string
   */
  static String getSplitStr(String& str,
                            int firstOccurrence,
                            int lastOccurrence) {
    // get the largest integer in the last pos
