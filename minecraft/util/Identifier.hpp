...
class Identifier {
 ...
 public:
  enum class Category { SOUND, BLOCK, LANG, ITEM, ENTITY, TEXTURE, GUI };
  ...
  explicit Identifier(const String& id, Category category) {
    ...
    if (!isNamespaceValid(strings.first)) {
      PLOG_ERROR << "Identifier " << id
                 << " namespace is invalid: " << strings.first;
    }
    if (!isPathValid(strings.second)) {
      PLOG_ERROR << "Identifier " << id
                 << " relativePath is invalid: " << strings.second;
    }
  ...
  explicit Identifier(const String& namespace_,
                      const String& path,
                      Category category) {
    if (!isNamespaceValid(namespace_)) {
      PLOG_ERROR << "Identifier " << namespace_ + ":" + path
                 << " namespace is invalid: " << namespace_;
    }
    if (!isPathValid(path)) {
      PLOG_ERROR << "Identifier " << namespace_ + ":" + path
                 << " relativePath is invalid: " << path;
    }
    this->category = category;
  ...
  /**
   * @example for id "minecraft:block.air_block", this returns
   * "minecraft:air_block"
  ...
  bool operator<(const Identifier& identifier) const {
    return (this->relativePath < identifier.relativePath &&
            this->namespace_ < identifier.namespace_);
  }
  ...
  static bool isNamespaceValid(const String& namespace_) {
    return std::all_of(namespace_.begin(), namespace_.end(),
                       Identifier::isNamespaceCharacterValid);
  }
  ...
  static bool isValid(const String& id) {
    ...
    return Identifier::isNamespaceValid(
               strings.first.empty() ? defaultNamespace : strings.first) &&
           Identifier::isPathValid(strings.second);
  }
 ...
 private:
  ...
  String _getAbsolutePath() {
    switch (category) {
      ...
      case Category::BLOCK:
        return Path::blockStatePath + getRelativePath() +
               Path::blockStateSuffix;
      case Category::LANG:
