 ...
 */
template <typename T>
static bool sign(T v) {
  return (v > 0);
}

static float constrain(float lower, float upper, float v) {
  if (v > upper)
    return upper;
  if (v < lower)
    return lower;
...
static int nthOccurrence(const std::string& str,
...
static Iter nthOccurrence(Iter first,
                          Iter last,
                          Iter first_,
                          Iter last_,
                          unsigned nth) {
  ...
  if (it == last)
    return it;
  return nthOccurrence(it + std::distance(first_, last_), last, first_, last_,
                       nth - 1);
}

template <typename T>
static T reverseSign(T v) {
  return -v;
}

template <typename T>
static std::string setPrecision(T number, short decimalPlace) {
  static_assert(std::is_floating_point<T>::value,
                "T must be either a floating point or integer type");
...
static std::pair<std::string, int> execCommand(const std::string& command) {
  ...
  std::string result;
  int return_code = -1;
  auto p_close_wrapper = [&return_code](FILE* cmd) {
    return_code = pclose(cmd);
  };
  {
    const std::unique_ptr<FILE, decltype(p_close_wrapper)> pipe(
        popen(cmd, "r"), p_close_wrapper);
    if (pipe) {
