...
class Date {
 ...
 public:
  static String getUTCBuildTime(const String& dateStr, const String& timeStr) {
    ...
    boost::posix_time::ptime utcTime = boost::date_time::local_adjustor<
        boost::posix_time::ptime, 0,
        boost::posix_time::no_dst>::utc_to_local(currentTime);
    // Format the UTC time into a single string
    std::string utcTimeString =
        boost::posix_time::to_iso_extended_string(utcTime);
