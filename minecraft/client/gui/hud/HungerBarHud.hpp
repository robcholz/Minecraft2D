...
namespace hud {
	...
	private:
		...
		void loadHunger(int number, sf::Texture* heartTexture) {
			...
			sprite_hunger->setPosition((float) getScreenCornerX() + (float) number * sprite_outline->getGlobalBounds().width + mapTo(1 - number),
		...
		void updatePosition() override {
			auto pos = convertToPixPos(screenWidth / 2.0f + mapTo(182 / 2 - 9 * 8), screenHeight - mapTo(21 + 9 + 10));
