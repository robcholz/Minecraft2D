...
class DebugHud : public GUI, public Hud {
 ...
 public:
  ...
  explicit DebugHud(MinecraftClientAccess* runCraftClientAccess,
    ...
    messageStackLeft.addMessage("pos", [&]() -> String {
      ...
      auto block_pos = this->worldAccess->getPlayer()
                           ->getEntityPosition()
                           .get<coordinate::BlockPos>();
      auto chunk_pos =
          chunk::Chunk::toChunkPosition(block_pos.x, block_pos.z).chunkPos;
      return (boost::format("XZ: %s / %s ChunkPos: %d") %
              utils::setPrecision(pos.x, 3) % utils::setPrecision(pos.z, 5) %
              chunk_pos)
          .str();
    });
    messageStackLeft.addMessage("block", [&]() -> String {
      auto pos = this->worldAccess->getPlayer()
                     ->getEntityPosition()
                     .get<coordinate::BlockPos>();
    ...
    messageStackLeft.addMessage("block_light", [&]() -> String {
      auto pos = this->worldAccess->getPlayer()
                     ...
                     .get<coordinate::BlockPos>();
      auto light =
          (int)this->worldAccess->getChunkManager()->getBlockLightLevel(pos);
    ...
    messageStackRight.addMessage("os", [&]() -> String {
      return (boost::format("System: %s %s %s") % OS::getOSName() %
              OS::getOSVersion() % OS::getOSArch())
          .str();
    });

    messageStackRight.addMessage(
        "cpu_model", [&]() -> String { return OS::getCPUModelName(); });
    messageStackRight.addMessage(
        "gpu_model", [&]() -> String { return OS::getGPUModelName(); });
    messageStackRight.addMessage(
        "gpu_model", [&]() -> String { return OS::getScreenInfo(); });
    messageStackRight.addMessage(
        "pid", [&]() -> String { return "PID: " + OS::getPID<String>(); });
    messageStackRight.addMessage("cpu_info", [&]() -> String {
      return (boost::format("CPU: %d%%") %
              (OS::getProcessCPUUsagePercent() / (int)OS::getProcessorCount()))
          .str();
    });
    messageStackRight.addMessage("mem_info", [&]() -> String {
      return (boost::format("Mem: %d%% %d/%dMB") %
              OS::getProcessMemoryUsagePercent() % OS::getProcessMemoryUsage() %
              OS::getTotalRAM())
          .str();
    });
  }
 ...
 private:
  class MessageStack {
   ...
   public:
    explicit MessageStack() { _layout.setLayout(Layout::LEFT | Layout::TOP); }
    ...
    void addMessage(const String& id, std::function<String()> func) {
      ...
      messageStack.push_back(std::move(
          std::make_tuple(id, 0.0f, std::move(message), std::move(func))));
    }
    ...
    void updateMessageHeight() {
      for (auto i = 0; i < messageStack.size(); ++i) {
        ...
        message->setTextPosition(getMessageOriginX(),
                                 getMessageOriginZ() + *height_current);
      }
    }
   ...
   private:
    ...
    Layout _layout{};
    std::vector<std::tuple<String,
                           float,
                           std::unique_ptr<RichText>,
                           std::function<String()>>>
        messageStack;
