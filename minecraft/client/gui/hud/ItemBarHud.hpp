...
namespace hud {
	...
	private:
		...
		void updatePosition() override {
			auto pos = convertToPixPos(screenWidth / 2.f - itemBarSprite.getGlobalBounds().width / 2,
		...
		void setPosition(HudPixelPosT x, HudPixelPosT z) override {
			Hud::setPosition(x, z);
			itemBarSprite.setPosition((float) getScreenCornerX(), (float) getScreenCornerZ());
		}
		...
		void focus(int number) {
			onFocusedItemSprite.setPosition(floor( mapTo((number * 20 - 1)) + (float) getScreenCornerX()),
