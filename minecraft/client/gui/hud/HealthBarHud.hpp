...
class HealthBarHud : public GUI, public Hud {
 ...
 public:
  explicit HealthBarHud(WorldAccess* worldAccess) {
    ...
    // (16,0) (24,8)
    darkOutlineHeartTexture.loadFromFile(iconAssetPath,
                                         sf::IntRect{16, 0, 9, 9});
    // (25,0) (33,8)
    whiteOutlineHeartTexture.loadFromFile(iconAssetPath,
                                          sf::IntRect{25, 0, 9, 9});
    // (53,1) (59,7)
    ...
    for (auto i = 0; i < MAX_HEARTS; ++i)
      loadHeartOutline(i, &darkOutlineHeartTexture);
    for (auto j = 0; j < MAX_HEARTS; ++j)
      loadHeart(j, &redHeartTexture);
 ...
 private:
  ...
  std::vector<
      ...
      healthBarSpritesContainer;  // 10 by default is 20 health value
    ...
    for (auto x = 0; x < 4; ++x) {
      for (int y = 0; y < 7; ++y) {
        combined_image.setPixel(
            x, y, halfRedHeartTexture.copyToImage().getPixel(x, y));
      }
    }
  ...
  void setHeartCapacity(int number) {
    for (auto i = 0; i < number; ++i) {
      ...
      healthBarSpritesContainer.emplace_back(std::move(sprite_outline),
    ...
    auto move_up_down = [&](int number, bool move) {
      ...
      if (move) {
        sprite_outline->setPosition(
            ...
            sprite_outline->getPosition().y - mapTo(1));  // move up
      } else {
  ...
  void updateHealth() {
    ...
    if (damaged)
      delay_timer = DAMAGE_EFFECT_DURATION;
