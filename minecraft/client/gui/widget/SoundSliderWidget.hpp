...
class SoundSliderWidget : public SliderWidget {
 public:
  explicit SoundSliderWidget(const std::string& id,
                             int width = 200,
                             int height = 20,
                             bool visible = true,
                             int x = 0,
    ...
    widgetSprite.setPosition((float)widgetOutlineBound.upper,
                             (float)widgetOutline.y);
  }

  explicit SoundSliderWidget(const std::string& words,
                             int defaultValue,
                             int width = 200,
                             int height = 20,
                             bool visible = true,
    ...
    widgetSprite.setPosition(
        (float)widgetOutlineBound.lower +
            ((float)defaultValue / 100) *
                (float)(widgetOutlineBound.upper - widgetOutlineBound.lower),
        (float)widgetOutline.y);
  }

  ~SoundSliderWidget() override = default;

  SoundSliderWidget& varToChangeWhenMoved(
      SoundManager::SoundVolume* controlledValue = nullptr) {
  ...
  void executeCallbackFunc() override {
    ...
    message.clear();
    sliderValue =
        (ButtonValue)(((float)(widgetOutline.x - widgetBackgroundOutline.x) /
                       (float)(widgetBackgroundOutline.width -
                               widgetOutline.width - 8)) *
                      100.0f);
