...
protected:
	...
	static std::wstring getDisplayContent(const std::wstring& ptitle,int val){
...
private:
	...
	void updateMessagePosition() {
		message.setPosition((float) widgetBackgroundOutline.x + (float) widgetBackgroundOutline.width / 2 - (float) message.getGlobalBounds().width / 2,
		                    (float) widgetBackgroundOutline.y + (float) widgetBackgroundOutline.height / 2 - (float) message.getGlobalBounds().height / 2);
	}
