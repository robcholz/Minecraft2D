...
class Widget : public GUI {
 protected:
  ...
  static void setOutline(Areai* outline,
  ...
  void loadWidgetTexture(const sf::IntRect& widgetNormal,
                         const sf::IntRect& widgetActivated,
                         const String& path) {
  ...
  virtual void setPressed(bool isPressed) { pressed = isPressed; }
  ...
  void setFocused(bool isFocused = true) {
    focus = isFocused;
    if (isFocused)
      widgetSprite.setTexture(widgetFocusedTexture);
    else
  ...
  virtual void onRender() { RenderSystem::render(widgetSprite); }

  virtual void onUpdate() {
    auto inInWidgetBoundary =
        checkVectorBoundary(getMousePosition(), widgetOutline);
 ...
 public:
  ...
  virtual void executeCallbackFunc() {
