...
#ifndef MINECRAFT_RICHTEXT_HPP
#define MINECRAFT_RICHTEXT_HPP
...
class RichText : public sf::Drawable, public sf::Transformable {
 public:
  ...
  class Line : public sf::Transformable, public sf::Drawable {
   public:
     ...
     * @note Attempting to access a character outside of the line bounds causes
     * a crash.
     * @param pos
     * @param color
     */
     ...
     * @note NOTE: Attempting to access a character outside of the line bounds
     * causes a crash
     * @param pos
     * @return
     */
   ...
   private:
     ...
     * \n Also changes pos to the widgetOutlinePosition of the character in the
     * sf::RichText.
     * @param pos
     * @return
     */
    ...
    void updateTextAndGeometry(sf::Text& text) const {
      ...
      // Update bounds
      float line_spacing =
          std::floor(text.getFont()->getLineSpacing(text.getCharacterSize()));
 ...
 private:
  ...
  sf::Color m_color = sf::Color(0, 0, 0, 0);
  ...
  void updateBackground() {
    ...
    background.setFillColor(m_color);
  }
 ...
 public:
  ...
  static std::vector<sf::String> explode(const sf::String& string,
                                         sf::Uint32 delimiter) {
    if (string.isEmpty())
      return {};
    ...
    // Add to the result if buffer still has contents or if the last character
    // is a delimiter
   ...
   * @note Attempting to access a character outside of the bounds causes a
   * crash.
   * @param line
   * @param pos
   * @param color
   ...
   * @param style
   */
  void setCharacterStyle(std::size_t line,
                         std::size_t pos,
                         sf::Text::Style style) {
    assert(line < lines.size());
  ...
  RichText& setBackgroundColor(const sf::Color& color = sf::Color::White) {
    this->m_color = color;
    return *this;
  }
