 ...
 public:
  explicit Background(const String& id) {
    ...
    if (!sourceTexture.loadFromFile(identifier->getAbsolutePath())) {
      PLOG_ERROR << ("CANNOT update sourceTexture from " +
                     identifier->getAbsolutePath());
    }
    textureAtlas.push_front(std::make_unique<sf::Sprite>(sourceTexture));
  }

  ~Background() { textureAtlas.clear(); }

  Background& fitToScreen() {
    textureAtlas.front()->scale(
        (float)RenderSystem::getScreenWidth() /
            static_cast<float>(sourceTexture.getSize().x),
        (float)RenderSystem::getScreenHeight() /
            static_cast<float>(sourceTexture.getSize().y));
    return *this;
  }
  ...
  Background& composeToScreen() {
    ...
    for (int image_x = 0; image_x < RenderSystem::getScreenWidth();
         image_x += texture_width) {
      for (int image_y = 0; image_y < RenderSystem::getScreenHeight();
           image_y += texture_width) {
