...
class Layout {
 public:
  ...
  explicit Layout(layout layout) { setLayout(layout); }

  void setLayout(layout layout) { this->_layout = layout; }

  void addLayout(layout layout) { this->_layout = _layout | layout; }

  /**
   * @brief elements layout
   * @note value assigned in binary so it's easy to use bitwise or (|) operator
   */
  static constexpr layout DEFAULT = 1
                                    << static_cast<layout>(Alignment::DEFAULT);
  static constexpr layout VCENTER = 1
                                    << static_cast<layout>(Alignment::VCENTER);
  static constexpr layout HCENTER = 1
                                    << static_cast<layout>(Alignment::HCENTER);
  ...
  static bool parse(const Layout& layout, Alignment alignment) {
    return ((layout.getLayout() >> static_cast<Layout::layout>(alignment)) &
            1) > 0;
  }
